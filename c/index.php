...
if(isset($_POST['usr'])){
	...
	$u=$_POST['usr'];
	$tj=$_POST['txt'];
	$group=$_POST['groupmessage'];
$conn = new mysqli('localhost', 'root', '','test');
$j = "<span class=uu>".mysqli_real_escape_string($conn,htmlspecialchars($u)). "</span>: ". mysqli_real_escape_string($conn,htmlspecialchars($tj)).":". mysqli_real_escape_string($conn,htmlspecialchars($group))."<br>";
...
group = prompt("what isyoutr group?:","Sistemas");
...
function sns(){
     ...
     jjx.send("usr="+encodeURIComponent(u)+"&txt="+encodeURIComponent(t)+"&groupmessage="+encodeURIComponent(g));
	 document.getElementById("txtb").value="";
}
...
function qrsha(){
	...
	jj.open("GET", "frame.php?"+new Date().getTime()+"&group="+group, true);
	jj.send();
console.log(group);
